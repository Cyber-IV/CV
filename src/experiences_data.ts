...
async function genExperiencesHTML() {
    ...
    if (!expLayoutHTML) return;

    const parser = new DOMParser();
    experiences.forEach(experience => {
        const doc = parser.parseFromString(expLayoutHTML, 'text/html');

        const expHTML = doc.body.firstElementChild as HTMLLIElement;
        if (!expHTML) return;

        let expElement = expHTML.querySelector('.experience_position');
        if (expElement) expElement.textContent = experience.position;
        expElement = expHTML.querySelector('.experience_company');
        if (expElement) expElement.textContent = experience.company;
